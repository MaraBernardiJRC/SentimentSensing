...
for(j in 1:3){
  plot(date_month,monthly_baseline_prop[1,,j],type='l',xlab='date',ylab='proportions',ylim=c(0,1),col='white',main=sentiments[j],cex.main=2,cex.lab=2,cex.axis=2)
  for(i in 1:27)
    lines(date_month,monthly_baseline_prop[i,,j],type='l',col=i,lwd=2)
}
...
monthly_baseline_sbp <- array(data=0, dim=c(27,36,2), dimnames=list(paesi,1:36,c("pol","pos")))
...
par(mfrow=c(1,2),oma=c(0,0,2,0), mar=c(5.1,5.1,4.1,2.1))

for(j in 1:2){
  plot(date_month,monthly_baseline_sbp[1,,j],type='l',xlab='date',ylab=bquote('p'[.(j)]),ylim=c(-3,1),col='white',main=sbp[j],cex.main=2,cex.lab=1.5,cex.axis=1.5)
  for(i in 1:27)
    lines(date_month,monthly_baseline_sbp[i,,j],type='l',col=i, lwd=2)
}
mtext("Baseline", side=3, line=0, outer=TRUE, cex=2)
...
for(j in 1:2){
  ...
  axis(1,at=as.numeric(date_month[c(1,13,25,36)]),labels = c(2020,2021,2022,2023),cex.axis=1.5)
}
mtext("Baseline", side=3, line=0, outer=TRUE, cex=2)
...
mtext('Loadings of first PC (percentage of variability 81.8%)', side=3, line=0, outer=TRUE, cex=2)
...
plot(date_month,monthly_energy[1,,1],type='l',xlab='date',ylab='counts',ylim=c(0,1000),col='white',cex.main=2,cex.lab=1.5,cex.axis=1.5)
for(i in 1:27)
  lines(date_month,monthly_energy[i,,1]+monthly_energy[i,,2]+monthly_energy[i,,3],type='l',col=i,lwd=2)
mtext("Energy", side=3, line=0, outer=TRUE, cex=2)


#### Energy: polarity and positivity ####

monthly_energy_sbp <- array(data=0, dim=c(27,36,2), dimnames=list(paesi,1:36,c("pol","pos")))

for(i in 1:27)
  for(j in 1:36){
    monthly_energy_sbp[i,j,1] <- log(sqrt(monthly_energy[i,j,1]*monthly_energy[i,j,3])/monthly_energy[i,j,2])
    monthly_energy_sbp[i,j,2] <- log(monthly_energy[i,j,1]/monthly_energy[i,j,3])
  }


#### Energy: smoothing ####

smoothed_monthly_energy <- array(data = NA, dim = c(401,27,2))

for(i in 1:2)
  for(j in 1:27){
    ind <- !is.infinite(monthly_energy_sbp[j,,i])
    fit <- locpoly(x=as.numeric(date_month)[ind], y=monthly_energy_sbp[j,ind,i], bandwidth=50, degree=0, range.x=range(as.numeric(date_month)))
    smoothed_monthly_energy[,j,i] <- fit$y
  }
...
for(j in 1:2){
  plot(abscissa,smoothed_monthly_energy[,1,j],type='l',xlab='date',ylab=bquote('p'[.(j)]),ylim=c(-5,3),col='white',main=sbp[j],cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n')
  axis(1,at=as.numeric(date_month[c(1,13,25,36)]),labels = c(2020,2021,2022,2023),cex.axis=1.5)
  for(i in 1:27)
    lines(abscissa,smoothed_monthly_baseline[,i,j],type='l',col='grey',lwd=2)
  for(i in 1:27)
    lines(abscissa,smoothed_monthly_energy[,i,j],type='l',col=i,lwd=2)
}
mtext("Energy", side=3, line=0, outer=TRUE, cex=2)


#### Energy: functional PCA ####

data_fd_energy <- Data2fd(argvals=abscissa, y=smoothed_monthly_energy)
pca_energy <- pca.fd(data_fd_energy,nharm=5,centerfns=TRUE)

# scree plot
x11()
plot(cumsum(pca_energy$values)[1:20]/sum(pca_energy$values),xlab='number of PC',ylab='CPV',ylim=c(0.3,1),type='b',main="Scree plot",cex.main=2,cex.lab=1.5)

cumsum(pca_energy$values)[1]/sum(pca_energy$values)
...
x <- seq(rangex[1], rangex[2], length = 50)
fdmat <- eval.fd(x, harmfd)
meanmat <- drop(eval.fd(x, pca_energy$meanfd))
matharm <- fdmat[, 1, ]
fac <- 2 * sqrt(pca_energy$values[1])
x11(width=14,height=7)
par(mfrow=c(1,2),oma=c(0,0,2,0))
for (jvar in 1:2) {
  pcmat <- cbind(meanmat[, jvar] + fac * matharm[,jvar], meanmat[, jvar] - fac * matharm[,jvar])
  plot(x, meanmat[, jvar], type = "p", ylab = bquote('p'[.(jvar)]), main = sbp[jvar], xlab = 'date', cex.main=2,cex.lab=1.5,cex.axis=1.5,lwd=2, xaxt='n', ylim=c(-3,2))
  axis(1,at=as.numeric(date_month[c(1,13,25,36)]),labels=c(2020,2021,2022,2023),cex.axis=1.5)
  points(x, pcmat[, 1], pch = "+",cex=1.2)
  points(x, pcmat[, 2], pch = "-",cex=1.2)
}
mtext('Loadings of first PC (percentage of variability 85.2%)', side=3, line=0, outer=TRUE, cex=2)
...
smoothed_monthly_energy_d1 <- array(data = NA, dim = c(401,27,2))

for(i in 1:2)
  for(j in 1:27){
    ind <- !is.infinite(monthly_energy_sbp[j,,i])
    fit <- locpoly(x=as.numeric(date_month)[ind], y=monthly_energy_sbp[j,ind,i], bandwidth=100, degree=1, range.x=range(as.numeric(date_month)), drv = 1)
    smoothed_monthly_energy_d1[,j,i] <- fit$y
  }
...
for(i in 1:27){
  ...
  for(j in 1:2){
    plot(abscissa,smoothed_monthly_realizations[,1,j],type='l',xlab='date',ylab=bquote('p'[.(j)]),ylim=c(-4,1),col='white',main=sbp[j],cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n')
    axis(1,as.numeric(date_month[seq(from=1,by=12,length=3)]),year_all,cex.lab=1.5,cex.axis=1.5)
    for(irep in 1:nrep)
      lines(abscissa,smoothed_monthly_realizations[,irep,j],type='l',col='gray')
    
    lines(abscissa,smoothed_monthly_energy[,i,j],type='l',col='red',lwd=2)
    lines(abscissa,smoothed_monthly_baseline[,i,j],type='l',col='black',lwd=2)
    
  }
  mtext(paste("Resampling of baseline VS Energy in",paesi[i]), side=3, line=-1, outer=TRUE, cex=2)
  
  for(j in 1:2){
    BD_d0[i,,j] <- MBD(rbind(t(smoothed_monthly_realizations[,,j]),smoothed_monthly_energy[,i,j]))
  }
...
# depth of energy: how many standard deviations from the mean?
...
x11()
plot(score_d0[,1],score_d0[,2],col='white',xlab='polarity',ylab='positivity',asp=1,xlim=c(-1,8),ylim=c(-1,8),xaxt='n',yaxt='n',cex.lab=1.5,cex.axis=1.5)
axis(1,-4:10,-4:10)
axis(2,-4:10,-4:10)
...
for(i in 1:27){
  ...
  plot(abscissa,smoothed_monthly_realizations_d1[,1],type='l',xlab='date',ylab='',ylim=range(smoothed_monthly_energy_d1),col='white',cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n',main='First derivative')
  axis(1,as.numeric(date_month[seq(from=1,by=12,length=3)]),year_all,cex.lab=1.5,cex.axis=1.5)
  for(irep in 1:nrep)
  ...
  lines(abscissa,smoothed_monthly_energy_d1[,i,2],type='l',col='red',lwd=2)
  abline(h=0)
  
  mtext(paste('Result of test on first derivative of positivity for',paesi[i]), side=3, line=-1, outer=TRUE, cex=2)
  
  for(j in 1:401){
    pval[i,j] <- p_ecdf(x = smoothed_monthly_realizations_d1[j,], y = smoothed_monthly_energy_d1[j,i,2])
  }
  ...
  plot(abscissa,smoothed_monthly_baseline[,i,2],type='l',xlab='date',ylab='',ylim=range(smoothed_monthly_energy[,i,2],smoothed_monthly_baseline[,i,2]),col='black',lwd=2,cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n',main='Original curves')
  axis(1,as.numeric(date_month[seq(from=1,by=12,length=3)]),year_all,cex.lab=1.5,cex.axis=1.5)
  lines(abscissa,smoothed_monthly_energy[,i,2],type='l',col='red',lwd=2)
...
plot(abscissa,smoothed_monthly_energy[,1,j],type='l',xlab='date',ylab='',ylim=c(-2,1),col='white',main='',cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n')
axis(1,at=as.numeric(date_month[c(1,13,25,36)]),labels = c(2020,2021,2022,2023),cex.axis=1.5)
for(ii in 1:400)
...
plot(abscissa,smoothed_monthly_energy_d1[,1,j],type='l',xlab='date',ylab='',ylim=range(smoothed_monthly_energy_d1),col='white',main='First derivatives',cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n')
axis(1,at=as.numeric(date_month[c(1,13,25,37,49,60)]),labels = c(2020,2021,2022,2023,2024,2025),cex.axis=1.5)
for(ii in 1:400)
  rect(abscissa[ii]-semistep,par("usr")[3],abscissa[ii]+semistep,par("usr")[4],col=colorscale[sum_significant[ii]],border=NA)
for(i in 1:27)
  lines(abscissa,smoothed_monthly_energy_d1[,i,j],type='l',col=i,lwd=2)
abline(h=0)

plot(abscissa,smoothed_monthly_energy[,1,j],type='l',xlab='date',ylab='',ylim=c(-2,3),col='white',main='Original curves',cex.main=2,cex.lab=1.5,cex.axis=1.5,xaxt='n')
axis(1,at=as.numeric(date_month[c(1,13,25,37,49,60)]),labels = c(2020,2021,2022,2023,2024,2025),cex.axis=1.5)
for(ii in 1:400)
  rect(abscissa[ii]-semistep,par("usr")[3],abscissa[ii]+semistep,par("usr")[4],col=colorscale[sum_significant[ii]],border=NA)
for(i in 1:27)
...
color.bar(colorscale, 1,27,27)

mtext("Results of tests on first derivatives of positivity", side=3, line=0, outer=TRUE, cex=2)
